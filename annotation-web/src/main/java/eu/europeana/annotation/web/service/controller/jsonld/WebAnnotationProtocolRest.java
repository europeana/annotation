...
public class WebAnnotationProtocolRest extends BaseRest {
	...
	// @RequestMapping(value = "/annotation/{provider}/{annotationNr}.jsonld",
	// method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	// @ResponseBody
	public ModelAndView getAnnotationLdByPath(@RequestParam(value = "wskey", required = false) String wskey,
			// @RequestParam(value = "profile", required = false) String
			// profile,
			@PathVariable(value = "provider") String provider, @PathVariable(value = "identifier") String identifier) {

		String action = "get:/annotationld/{provider}/{annotationNr}.jsonld";
		return getAnnotation(wskey, provider, identifier, action);
	}

	// @RequestMapping(value = "/annotation.jsonld?provider=&annotationNr=",
	// method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	// @RequestMapping(value = "/annotation.jsonld", method = RequestMethod.GET,
	// produces = MediaType.APPLICATION_JSON_VALUE)
	// @ResponseBody
	public ModelAndView getAnnotationLd(@RequestParam(value = "apiKey", required = false) String apiKey,
	...
	private ModelAndView getAnnotation(String apiKey, String provider, String identifier, String action) {
		...
		try {
			...
			Annotation resAnnotation = annotationBuilder.copyIntoWebAnnotation(annotation);
			...
			return getValidationReport(apiKey, action,
					AnnotationOperationResponse.ERROR_VISIBILITY_CHECK + ". " + e.getMessage(), e);
		} catch (Exception e) {
			getLogger().error("An error occured during the invocation of :" + action, e);
			return getValidationReport(apiKey, action,
					AnnotationOperationResponse.ERROR_NO_OBJECT_FOUND + ". " + e.getMessage(), e);
		}
	}

	// @RequestMapping(value = "/annotation/{motivation}.jsonld", method =
	// RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	// @ResponseBody
	// @ApiOperation(notes=WebAnnotationFields.SAMPLES_JSONLD_LINK, value="")
//	public ModelAndView createAnnotationLd(@RequestParam(value = "wskey", required = false) String wskey,
//			@PathVariable(value = "motivation") String motivation,
//			@RequestParam(value = "provider", required = false) String provider, // this
//																					// is
//																					// an
//																					// ID
//																					// provider
//			@RequestParam(value = "identifier", required = false) String identifier,
//			@RequestParam(value = "indexing", defaultValue = "true") boolean indexing, @RequestBody String annotation) {
//
...
//
//			// check whether annotation vor given provider and annotationNr
//			// already exist in database
//			if (getAnnotationService().existsInDb(annoId))
//				return getValidationReport(wskey, action,
//						AnnotationOperationResponse.ERROR_ANNOTATION_EXISTS_IN_DB + annoId.toUri(), null);
//
...
//			 */
//			Annotation resAnnotation = annotationBuilder.copyIntoWebAnnotation(storedAnnotation);
//
//			JsonLd annotationLd = new EuropeanaAnnotationLd(resAnnotation);
//			String jsonLd = annotationLd.toString(4);
//			return JsonWebUtils.toJson(jsonLd, null);
...
//	}
	...
	// @RequestMapping(value = "/annotations/search.jsonld", method =
	// RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	// @ResponseBody
	// @ApiOperation(notes=WebAnnotationFields.SEARCH_NOTES, value="")
	public ModelAndView searchLd(@RequestParam(value = "wsKey", required = false) String wsKey,
		...
		response = buildSearchResponse(annotationList, wsKey, "/annotations/search.jsonld");
	...
	public ResponseEntity<String> createAnnotation(@RequestParam(value = WebAnnotationFields.WSKEY) String wskey,
			...
			@RequestHeader(value = WebAnnotationFields.USER_TOKEN, required = false, defaultValue = USER_ANONYMOUNS) String userToken)
					throws ParamValidationException, UserAuthorizationException, RequestBodyValidationException,
					InternalServerException {

		return storeAnnotation(null, provider, identifier, indexOnCreate, annotation, userToken);
	}
...
//					WebAnnotationFields.PATH_PARAM_FORMAT, format, HttpStatus.NOT_ACCEPTABLE, null);
		...
		return storeAnnotation(motivation, provider, identifier, indexOnCreate, annotation, userToken);
	}
	
	protected ResponseEntity<String> storeAnnotation(MotivationTypes motivation,  String provider, String identifier, boolean indexOnCreate,
			String annotation, String userToken) throws RequestBodyValidationException, InternalServerException, ParamValidationException {
		try {
