...
public class BaseJsonldRest extends BaseRest {
  ...
  protected ResponseEntity<String> storeAnnotation(MotivationTypes motivation,
    ...
    try {
      ...
      String clientId = verifyClient(authentication);
  ...
  private String verifyClient(Authentication authentication)
  ...
  protected ResponseEntity<String> storeAnnotations(String annotationPageIn,
    ...
    try {
      ...
      getAnnotationService().validateWebAnnotations(annotations, uploadStatusValidation,
          authentication);
      ...
      if (annosWithId.size() != existingInDb.size()) {
        ...
        getAnnotationService().reportNonExisting(annotations, uploadStatusNonExist,
            annoIdentifiers);
        throw new BatchUploadException(uploadStatusNonExist.toString(), uploadStatusNonExist,
      ...
      if (annosWithId.getAnnotations().size() > 0) {
        ...
        getAnnotationService().updateExistingAnnotations(uploadStatusExist,
      ...
      if (annosWithoutId.size() > 0) {
        ...
        getAnnotationService().insertNewAnnotations(uploadStatusInsert,
            annosWithoutId.getAnnotations(), annoDefaults, webAnnoStoredAnnoAnnoMap);
      }
  ...
  protected ResponseEntity<String> updateAnnotation(long identifier, String annotation,
    ...
    try {
      ...
      //need to validate client information as well
