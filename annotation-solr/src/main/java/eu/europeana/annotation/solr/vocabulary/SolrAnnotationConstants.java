package eu.europeana.annotation.solr.vocabulary;

public interface SolrAnnotationConstants {
	
	public static final String ANNOTATION_ID_URL = "annotation_id_url";
	public static final String TARGET_URL = "annotation_id_url";
	public static final String TARGET_RECORD_ID = "annotation_id_url";
	public static final String MOTIVATON_KEY = "annotation_id_url";
	
	public static final String BODY_VALUE = "body_value";
	public static final String BODY_TEXT  = "body_text";
	public static final String BODY_INTERNAL_TYPE_KEY = "body_internal_type";
	public static final String INTERNAL_TYPE_KEY = "internal_type_key";	
	public static final String MODERATION_SCORE = "moderation_score";
	public static final String UPDATED_TIMESTAMP = "updated_timestamp";
	public static final String ANNOTATION_ID_URL2 = "annotation_id_url";
	public static final String BODY_TAG_ID = "body_tag_id";
	public static final String TARGET_INTERNAL_TYPE_KEY = "target_internal_type_key";
	public static final String MOTIVATION_KEY = "motivation_key";
	public static final String TARGET_RECORD_IDS = "target_record_ids";
	public static final String TARGET_URLS = "target_urls";
	public static final String SERIALIZED_AT_TIMESTAMP = "serialized_at_timestamp";
	public static final String ANNOTATED_AT_TIMESTAMP = "annotated_at_timestamp";
	
	public static final String ANNOTATED_AT = "annotatedAt";
	public static final String SERIALIZED_AT = "serializedAt";
	public static final String MODIFIED = "modified";
	public static final String MODERATIONSCORE = "moderationScore";
	
	public static final int DEFAULT_FACET_LIMIT = 100;
}